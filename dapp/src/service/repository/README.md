...
This directory contains a provider-agnostic Git repository abstraction that works with any Git remote (GitHub, GitLab, Bitbucket, Azure DevOps, self-hosted, etc.) over HTTPS/SSH, without depending on provider-specific APIs.
...
### Universal Support
- ✅ **Any Git remote** accessible via HTTPS/SSH
- ✅ **Self-hosted instances** of supported providers
- ✅ **Custom Git servers** with standard Git protocol
...
The system supports multiple credential formats:
...
export CI_JOB_TOKEN="xxxxxxxxxxxx"          # CI environment
...
export AZURE_PAT="xxxxxxxxxxxx"
export ADO_TOKEN="xxxxxxxxxxxx"

# Generic (applies to all providers)
export GIT_TOKEN="xxxxxxxxxxxx"
export SCM_TOKEN="xxxxxxxxxxxx"

# SSH Keys
export SSH_PRIVATE_KEY_PATH="/path/to/key"
export SSH_PASSPHRASE="key_passphrase"
...
// Add credentials for specific hosts
credManager.addCredentials("github.com", {
  type: "https",
  username: "token",
  tokenOrPassword: "ghp_xxxxxxxxxxxx"
});

credManager.addCredentials("gitlab.example.com", {
  type: "ssh", 
...
// Bulk configuration
credManager.loadCredentialsFromConfig({
  "bitbucket.org": {
    type: "https",
    username: "x-token-auth", 
    tokenOrPassword: "app_password"
  }
});
```
...
**Advantages:**
- 🚀 **Performance**: Partial clones, sparse-checkout, efficient operations
- 🔧 **Full Git Features**: Supports all Git operations and configurations  
- 🛡️ **Security**: Uses system Git configuration and SSH agents
- 📊 **Rich Metadata**: Access to complete Git object database
...
- ⚡ **Quick Setup**: No Git CLI installation required
- 🔌 **API Rich**: Can access provider-specific metadata
...
const testProviders = [
  ...
  { name: "Bitbucket", url: "https://bitbucket.org/atlassian/stash-example-plugin" },
];
...
for (const provider of testProviders) {
  ...
  const info = await repo.getRepositoryInfo();
  console.log(`✅ ${provider.name}: ${info.defaultBranch}`);
}
```
...
- 🔐 **Credential Isolation**: Each host gets separate credentials
- 🛡️ **No Credential Logging**: Debug info never exposes tokens/keys
- 🔑 **SSH Agent Support**: Integrates with system SSH configuration
- 🚫 **Sandboxed Execution**: Git operations run in controlled environment
